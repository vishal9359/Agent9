...
- Escape special characters for Mermaid: brackets () {} [] are now escaped as ASCII codes (#40; #41; etc.)
- Function calls now include parameters: foo(param) instead of foo()
- Return statements show the actual return value/expression instead of generic "return from function"
- Multi-line statements use semicolons instead of <br/> tags for better rendering
- Condition labels show actual conditions (e.g., "check x >= 10") instead of generic "check()"
"""
...
def _escape_mermaid_chars(text: str) -> str:
    """
    Escape special characters for Mermaid using ASCII codes.
    Mermaid doesn't allow brackets in node labels, so we use HTML entity codes:
    ( = #40;  ) = #41;  { = #123;  } = #125;  [ = #91;  ] = #93;
    """
    if not text:
        return text
    # Escape brackets with ASCII codes
...
def _extract_params(params_str: str) -> str:
    ...
    Remove complex expressions and keep simple identifiers.
    """
    if not params_str:
        return ""
    ...
    # Keep only simple identifiers or shortened expressions
    simplified = []
    for param in params:
        # Extract just variable names, not complex expressions
        # Look for last identifier in the parameter
        tokens = re.findall(r'\b[A-Za-z_][A-Za-z0-9_]*\b', param)
        if tokens:
            # Use the last meaningful token (usually the variable name)
            simplified.append(tokens[-1])
        elif param:
            # Keep short parameters as-is
            if len(param) <= 15:
                simplified.append(param)
    
    return ", ".join(simplified[:3])  # Limit to 3 params for readability


def _replace_function_calls(text: str, for_condition: bool) -> str:
    """
    Replace function calls with Mermaid-safe representation including parameters.
    - Extracts and shows function parameters
    - Escapes special characters using ASCII codes
    """
    if not text:
        return text
    ...
    # Match method calls first (with arrow or dot)
    text = re.sub(
...
def clean_label_text(label: str, for_condition: bool = False) -> str:
    ...
    - Escape special characters that can break Mermaid
    """
    label = clean_unicode_chars(label or "")

    # Replace function-call syntax first (keep params, will be escaped later)
    ...
    # Convert arrow to dot before escaping
    label = label.replace("->", ".")
    
    # Remove statement terminators and question marks
    label = re.sub(r"[;?]", " ", label)
    
    # Replace <br/> with semicolon + space for better rendering
    # The <br/> tag causes issues with long variable names
    label = label.replace("<br/>", "; ")
    label = label.replace("<br>", "; ")
    
    # Normalize whitespace before escaping
    label = re.sub(r"\s+", " ", label).strip()
    
    # Escape special characters for Mermaid (must be done AFTER all text processing)
...
def clean_condition_text(label: str) -> str:
    ...
    Removes leading control keywords and outer parentheses while preserving the actual condition.
    """
    label = clean_unicode_chars(label or "")
    
    # Remove leading control keywords
    label = re.sub(r"^\s*(if|for|while|switch)\b", "", label, flags=re.IGNORECASE).strip()
    
    # Remove outer parentheses from conditions like "if (condition)" -> "condition"
    # But keep nested parentheses in function calls
    label = label.strip()
...
def _summarize_once(block_text: str) -> str:
    ...
    prompt = (
        ...
        "\n"
        "Rules:\n"
        "- Output a single concise line (max 120 chars)\n"
        "- If multiple statements, separate with semicolons like: 'x = y; call foo()'\n"
        "- Keep important assignments and calls\n"
        "- Include function parameters when calling functions: 'foo(param)'\n"
        "- Do NOT output control-flow keywords (if/for/while/switch/try/catch)\n"
        "- Do NOT use line breaks, <br/>, or newlines\n"
        ...
        "{block}\n"
        "\n"
        "Pseudo-code label (one line, semicolon-separated):"
    )
...
class FlowBuilder:
    ...
    def build_stmt(self, cursor) -> tuple[str, list[str]]:
        ...
        if k == cindex.CursorKind.RETURN_STMT:
            ...
            # Remove the "return" keyword to get just the expression
            return_expr = re.sub(r"^\s*return\s+", "", return_text, flags=re.IGNORECASE).strip()
            return_expr = re.sub(r";+\s*$", "", return_expr).strip()  # Remove trailing semicolons
            
            if return_expr:
                label = f"return {return_expr}"
            else:
                label = "return"
    ...
    def _build_if(self, cursor) -> tuple[str, list[str]]:
        ...
        # Only add "check" prefix if the condition is empty or very short
        # Otherwise use the condition directly
        if not cond_label or cond_label == "condition":
            display_label = "check condition"
        else:
            display_label = f"check {cond_label}"
        
        d = self.new_node("decision", display_label)
    ...
    def _build_loop(self, cursor) -> tuple[str, list[str]]:
        ...
        # Only add "check" prefix if the condition is empty or very short
        if not cond_label or cond_label == "loop condition":
            display_label = "check loop condition"
        else:
            display_label = f"check {cond_label}"
        
        check = self.new_node("decision", display_label)
    ...
    def _build_range_for(self, cursor) -> tuple[str, list[str]]:
        ...
        # Use "for each" for range-based loops
        if cond_label and cond_label != "range":
            display_label = f"for each {cond_label}"
        else:
            display_label = "for each item"
    ...
    def _build_switch(self, cursor) -> tuple[str, list[str]]:
        ...
        # Use "switch on" for switch statements
        if expr_label and expr_label != "expression":
            display_label = f"switch on {expr_label}"
        else:
...
def validate_mermaid(mermaid: str) -> tuple[bool, Optional[str]]:
    ...
    # Allow #40; style ASCII codes (not &#40; HTML entities)
    if "&#" in mermaid:
        return False, "HTML entities detected (use #40; format instead)"
